...
const get = (name) => {
  try {
    // Run tar from the tarball's directory with a relative name: an absolute
    // Windows path (C:\...) is parsed as tar's remote-host syntax and fails.
    return execFileSync('tar', ['-xOzf', path.basename(tgz), name], { cwd: path.dirname(tgz), maxBuffer: 32 * 1024 * 1024 }).toString('utf8');
